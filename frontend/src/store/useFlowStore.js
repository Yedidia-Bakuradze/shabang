...
const useFlowStore = create((set, get) => ({
  ...
  // Normalization State
  normalizedDSD: null,        // Normalized DSD result from backend
  pendingNormalization: null, // Normalization waiting for approval
  ...
  setNormalizedDSD: (normalizedDSD) => {
    set({ normalizedDSD, pendingNormalization: normalizedDSD });
  },

  applyNormalizedSchema: () => {
    const pending = get().pendingNormalization;
    if (pending) {
      // Apply the normalized schema as the new DSD
      const { nodes: dsdNodes, edges: dsdEdges } = get().generateDSDNodesAndEdges(pending);
      set({ 
        dsdData: pending, 
        dsdNodes, 
        dsdEdges,
